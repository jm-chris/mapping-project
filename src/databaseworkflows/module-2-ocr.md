...
### 1. OCRmyPDF (Best for Accessible Batch Processing)
...
### 2. PaddleOCR (Best for High-Accuracy Chinese)
...
### 3. Transkribus (Best for Manuscripts and Tibetan/Mongolian)
...
### 4. ABBYY FineReader (Best Commercial Option)
...
*   **Hallucination Risk:** If the OCR text is too degraded, the LLM may "hallucinate" and invent plausible-sounding historical text to fill the gaps [6]. 
*   **Verification:** LLM post-correction should be treated as a first draft. Researchers must still verify the corrected text against the original image, particularly for dates, names, and numbers, which LLMs are prone to "correcting" erroneously.
